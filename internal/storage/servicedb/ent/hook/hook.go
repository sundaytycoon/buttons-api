...
// The UserOAuthProviderFunc type is an adapter to allow the use of ordinary
// function as UserOAuthProvider mutator.
type UserOAuthProviderFunc func(context.Context, *ent.UserOAuthProviderMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserOAuthProviderFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.UserOAuthProviderMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserOAuthProviderMutation", m)
	}
	return f(ctx, mv)
}
